...
- `gpc-utils.js`：Plus 支付相关纯工具函数；负责 Plus 支付方式归一化、GPC 页面地址归一化、卡密格式校验、卡密余额查询 URL 与余额响应解析。
...
- `flows/openai/background/steps/create-plus-checkout.js`：Plus 模式第 6 步实现，负责在已登录 ChatGPT 页面创建 PayPal checkout session，或在 GPC 模式下打开页面、覆盖卡密输入框、读取完整 ChatGPT session 并填入页面，记录页面准备运行态。
...
- `flows/openai/background/steps/fill-plus-checkout.js`：Plus 模式第 7 步实现，负责驱动 PayPal checkout 页面选择付款方式、填写账单地址、提交订阅并等待跳转；GPC 模式则接管 GPC 页面，点击 `开始plus充值`，按右侧日志和按钮文案等待订阅完成、无试用资格、失败或再次启动。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部新增“贡
  ...
  钥配置行，ChatGPT2API 额外提供 URL、Admin Key 与上传状态行；设置卡片新增 `IP代理` 开关与代理配置折叠区，支持 711Proxy 账号密码模式、代理状态卡与出口检测按钮；接码卡内把注册方式、接码配置、接码订单运行态与“注册手机号”身份运行态分层展示，“注册手机号”位于订单运行状态下方，不嵌入当前分配/验证码网格；设置卡片新增 `Plus 模式` 开关，并在 `Plus 支付` 之上新增 `账号接入策略` 下拉；该下拉只显示 `OAuth / 使用会话 JSON 导入` 两种接入方式，会话导入目标由当前来源自动决定，不支持的来源会直接禁用并显示“当前来源仅支持 OAuth”；PayPal 账号下拉框继续使用公共表单弹窗添加账号；GPC Plus 配置只提供 `GPC 卡密` 输入、卡密状态提示与 `购买卡密` 入口；Hotmail / 2925 两个账号池当前都使用统一的头部“添加账号/取消添加”按钮和共享表单容器；注册邮箱下方新增 `执行范围` 行，目前只在 codex/openai flow 显示，用于配置允许执行的起止步骤。
...
- `tests/gpc-utils.test.js`：测试 Plus 支付方式归一化，以及 GPC 页面地址、卡密格式、卡密余额 URL 和余额解析。
- `tests/plus-checkout-address-input.test.js`：测试 Plus checkout 内容脚本的国家/商户路径、结构化地址识别和付款方式判定。
- `tests/plus-checkout-create-wait.test.js`：测试 Plus checkout 创建节点的等待策略，以及 GPC 页面准备、卡密填入和 ChatGPT session 写入。
