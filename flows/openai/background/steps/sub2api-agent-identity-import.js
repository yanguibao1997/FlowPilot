(function attachBackgroundSub2ApiAgentIdentityImport(root, factory) {
  ...
  function createSub2ApiAgentIdentityImportExecutor(deps = {}) {
    ...
    async function getResolvedSessionTab(tabId, visibleStep) {
      ...
      if (!tab?.id) {
        throw new Error(`步骤 ${visibleStep}：ChatGPT 会话标签页不存在或已关闭，无法继续导入 Agent Identity。`);
      }
    ...
    async function executeSub2ApiAgentIdentityImport(state = {}) {
      ...
      await addStepLog(visibleStep, '正在定位当前 ChatGPT 会话页并准备注册 Agent Identity...', 'info');
      ...
      await addStepLog(visibleStep, '正在读取当前 ChatGPT 登录会话以注册 Agent Identity...', 'info');
