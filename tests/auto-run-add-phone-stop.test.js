...
test('auto-run retryable failure keeps retry state out of persisted account history', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  let currentState = {
    ...
    accountIdentifier: 'retry@example.com',
  ...
  const controller = api.createAutoRunController({
    ...
    runAutoSequenceFromStep: async () => {
      ...
      currentState = {
        ...currentState,
        accountIdentifierType: 'email',
        accountIdentifier: 'retry-2@example.com',
        email: 'retry-2@example.com',
      };
    },
    runtime,
    setState: async (updates = {}) => {
...
test('auto-run controller stops all rounds on DuckDuckGo daily limit even when skip failures is enabled', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  let currentState = {
    ...
    autoRunRoundSummaries: [],
    accountIdentifierType: 'email',
    accountIdentifier: 'duck-limit@example.com',
    email: 'duck-limit@example.com',
  };
  ...
  const controller = api.createAutoRunController({
    ...
    hasSavedProgress: () => false,
    isAddPhoneAuthFailure: () => false,
    isDuckDdgDailyLimitFailure: (error) => /DDG_DAILY_LIMIT::/.test(error?.message || String(error || '')),
    isPhoneSmsPlatformRateLimitFailure: () => false,
    isRestartCurrentAttemptError: () => false,
    isSignupUserAlreadyExistsFailure: () => false,
    isStopError: () => false,
    ...
    runAutoSequenceFromStep: async () => {
      events.runCalls += 1;
      throw new Error('DDG_DAILY_LIMIT::DuckDuckGo 地址生成已达到每日限制：daily limit reached');
    },
    runtime,
    setState: async (updates = {}) => {
      currentState = {
  ...
  assert.equal(events.accountRecords.length, 1);
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /DDG_DAILY_LIMIT::/);
  assert.equal(events.stopBroadcasts, 1);
  assert.ok(events.cancels.some((reason) => /DuckDuckGo 地址生成每日限制/.test(reason)));
  assert.ok(events.logs.some(({ message, level }) => level === 'error' && /DuckDuckGo 地址生成每日限制/.test(message)));
  assert.ok(events.logs.some(({ message }) => /自动运行已停止/.test(message)));
  assert.equal(runtime.state.autoRunActive, false);
  assert.equal(runtime.state.autoRunSessionId, 0);
});
