...
- `openai`：OpenAI / ChatGPT OAuth 注册、登录、平台绑定、Plus 扩展链路，以及 `webchat / ChatGPT2API` 私有远程会话上传链路
...
- 轮询登录验证码（邮箱或短信）
- 自动确认 OAuth 同意页
- 把 localhost 回调提交到 CPA、SUB2API 或 Codex2API，或在远程上传 target 下把当前 ChatGPT 会话上传到 `webchat / ChatGPT2API`
...
- 展示 `Plus 模式` 开关、`账号接入策略` 与 `Plus 支付` 配置；`账号接入策略` 固定放在 `Plus 支付` 上方，只提供 `OAuth / 使用会话 JSON 导入` 两种接入方式，会话导入的 CPA / SUB2API 目标由上方 `来源` 自动决定，不支持会话导入的来源会直接禁用为仅 OAuth；支付方式支持 PayPal / 无卡直绑 / GPC / 无需支付，PayPal 展示账号池下拉与添加按钮，无卡直绑展示 hosted checkout 相关配置，GPC 只展示卡密输入、购买卡密入口和卡密状态提示，无需支付隐藏支付链路配置；步骤列表会按支付方式、账号接入策略和远程上传 target 动态切换，普通模式的注册成功等待步骤不再显示或执行；Plus 模式下手机号注册不可用，只允许邮箱注册
...
- `ChatGPT2API` 目标在 OpenAI flow 内显示独立的 URL、Admin Key 与上传状态行；它不复用共享 `webchat/webchat2api` 输入，也不显示 OpenAI 接码设置
...
[core/flow-kernel/flow-registry.js](./core/flow-kernel/flow-registry.js) 定义 flow、来源、可见设置分组、runtime source 与 driver。
[core/flow-kernel/source-registry.js](./core/flow-kernel/source-registry.js) 负责把 flow runtime source 与共享 mail source 合并成统一来源视图。
[core/flow-kernel/flow-capabilities.js](./core/flow-kernel/flow-capabilities.js) 负责把当前 flow/source 解析成 sidepanel 的能力矩阵。
[core/flow-kernel/settings-schema.js](./core/flow-kernel/settings-schema.js) 负责把旧平铺字段与新的 `flows.*` 配置结构互转。
...
- OpenAI 的 target capability 还统一决定 Plus 模式下 `账号接入策略` 的可选项：`cpa` 支持 `oauth / cpa_codex_session`，`sub2api` 支持 `oauth / sub2api_codex_session`，`codex2api / webchat / chatgpt2api` 当前仅支持 `oauth`
- OpenAI 的 `webchat / chatgpt2api` 都是该 flow 私有远程上传 target：它们会替换 OAuth 平台绑定尾链，在注册成功等待后读取当前 ChatGPT 会话并上传，不等同于 Grok 的 `webchat2api` SSO Cookie 上传，也不是公开贡献 / 发布 adapter
...
[core/flow-kernel/runtime-state.js](./core/flow-kernel/runtime-state.js) 则负责把 session 运行态整理成统一的结构化视图与 patch 入口：
...
[data/step-definitions.js](./data/step-definitions.js) 提供共享步骤元数据，并按当前 `activeFlowId` 输出 flow-specific workflow。`openai` flow 会继续按 `plusPaymentMethod / signupMethod / plusAccountAccessStrategy / targetId` 动态解析步骤标题与节点集合：当 Plus 模式为邮箱注册且来源支持时，原本的 OAuth 尾链可以被替换成 `sub2api-session-import` 或 `cpa-session-import`；当 target 为 `webchat` 或 `chatgpt2api` 时，会省略 OAuth 登录、登录验证码、后置手机号、OAuth 确认与平台验证尾链，并追加对应的会话上传节点；`kiro` flow 输出固定的 9 节点注册/桌面授权/上传 workflow；`grok` flow 输出固定的 6 节点注册、SSO Cookie 提取与 `webchat2api` 上传 workflow。
...
- `core/flow-kernel/runtime-state.js` 会把会话运行态统一整理成 `runtimeState / sharedState / serviceState / flowState` 四层结构；当前顶层那些 `currentNodeId / nodeStatuses / automationWindowId / ipProxyApplied* / flowId / runId` 等兼容字段，都是这份结构化运行态的投影视图，不应再当成另一套平行状态机维护。
...
- 当前 flow 与目标配置：`activeFlowId`、OpenAI 的 `panelMode`（归一到 integration target）、Kiro/Grok 的 `flows.<flowId>.targetId`
...
- Codex2API 配置
- OpenAI `webchat / ChatGPT2API` 远程上传 target 配置
...
- Plus 支付方式 `plusPaymentMethod`，GPC 页面充值配置 `gpcBaseUrl / gpcCardKey / gpcBalance / gpcBalancePayload / gpcBalanceUpdatedAt / gpcBalanceError / gpcRemainingUses / gpcCardStatus / gpcPageStatus / gpcPageStatusText`；其中 GPC 页面地址固定归一为 `https://gpc.qlhazycoder.top`
...
- `panelMode` 现在只代表 `openai` flow 下的 legacy integration target，当前可归一到 `cpa | sub2api | codex2api | webchat | chatgpt2api`；`kiro / grok` flow 使用独立的 `flows.<flowId>.targetId`
...
[core/flow-kernel/tab-runtime.js](./core/flow-kernel/tab-runtime.js) 负责：
...
- 它会在 `content/utils.js` 之后注入到需要真实页面交互的 bundle，例如 `openai-auth`、`plus-checkout`、`paypal-flow`，以及 `mail-2925` 这类仍有页面级点击/清理动作的链路。
...
- [flows/openai/background/steps/open-chatgpt.js](./flows/openai/background/steps/open-chatgpt.js)
...
- [flows/openai/background/steps/submit-signup-email.js](./flows/openai/background/steps/submit-signup-email.js)
...
- [flows/openai/background/steps/fill-password.js](./flows/openai/background/steps/fill-password.js)
...
- [flows/openai/background/steps/fetch-signup-code.js](./flows/openai/background/steps/fetch-signup-code.js)
- [flows/openai/background/steps/fetch-login-code.js](./flows/openai/background/steps/fetch-login-code.js)
...
- 手动点击 Step 4 重新执行时，后台会先检查 `openai-auth` 认证页标签是否仍然存在；如果步骤 1 / 2 打开的认证页已经关闭，就会直接提示“请先执行步骤 1 或步骤 2，确保认证页仍然打开并停留在验证码页”，不会先重置后续步骤再报技术错误。
...
- [flows/openai/background/steps/fill-profile.js](./flows/openai/background/steps/fill-profile.js)
...
- [flows/openai/background/steps/wait-registration-success.js](./flows/openai/background/steps/wait-registration-success.js)
...
- [flows/openai/background/steps/oauth-login.js](./flows/openai/background/steps/oauth-login.js)
...
- [flows/openai/background/steps/confirm-oauth.js](./flows/openai/background/steps/confirm-oauth.js)
...
- [flows/openai/background/steps/platform-verify.js](./flows/openai/background/steps/platform-verify.js)
...
- SUB2API 主链路不再打开后台页面，也不再依赖 `flows/openai/content/sub2api-panel.js` 的 DOM 自动化；该内容脚本仅保留为旧后台页面路径的兼容能力。
...
### OpenAI 远程上传 target 补充

OpenAI flow 的 `webchat` 与 `chatgpt2api` target 都属于 OpenAI 自己的私有远程收尾目标。它们与 CPA / SUB2API / Codex2API 的 OAuth 平台绑定不同，也不同于 Grok flow 的 `webchat2api` SSO Cookie 上传。

共同流程：

1. sidepanel 仍切在 `openai` flow，只是把来源切到 `webchat` 或 `ChatGPT2API`
2. capability 层隐藏 OpenAI 接码配置，并要求对应 target 的 URL / Admin Key 配置完整
3. workflow 保留注册前链：打开 ChatGPT、提交注册邮箱、填写密码、获取注册验证码、填写资料、等待注册成功
4. workflow 移除 OAuth 登录、登录验证码、绑定邮箱、后置手机号、OAuth 确认和 `platform-verify`
5. 注册成功等待后追加对应上传节点，由后台读取当前 ChatGPT session 和 accessToken，再上传到远端
6. 上传状态回写到各自独立字段，并由 sidepanel 渲染上传状态和目标 URL

`ChatGPT2API` 目标的具体协议：

- 配置字段独立保存在 `settingsState.flows.openai.targets.chatgpt2api.baseUrl / apiKey`，兼容平铺字段 `openaiChatgpt2ApiUrl / openaiChatgpt2ApiAdminKey`
- 上传器为 [flows/openai/background/publisher-chatgpt2api.js](./flows/openai/background/publisher-chatgpt2api.js)
- URL 只取配置地址的 origin，并固定拼成 `${origin}/api/accounts`
- 请求方法为 `POST`
- 请求头包含 `Content-Type: application/json`、`Accept: application/json`、`Authorization: Bearer <Admin Key>`
- payload 为 `{ "tokens": ["<ChatGPT accessToken>"] }`
- 成功后写入 `openaiChatgpt2ApiUploadStatus = uploaded`、`openaiChatgpt2ApiUploadedAt`、`openaiChatgpt2ApiUploadMessage`、`openaiChatgpt2ApiTargetUrl`
- 缺 URL、缺 Admin Key、缺 ChatGPT session accessToken 或远端返回失败都必须报错并写入 `openaiChatgpt2ApiUploadStatus = error`

边界：

- `ChatGPT2API` 不是 Grok flow 的 `webchat2api` target，不读取也不上传 Grok SSO Cookie
- `ChatGPT2API` 不复用共享 webchat 输入，避免 OpenAI `webchat`、Grok `webchat2api` 和 OpenAI `ChatGPT2API` 三者配置互相覆盖
- `ChatGPT2API` 不是公开贡献 / 发布 adapter；如果后续要纳入贡献体系，必须先设计 publication target、贡献 adapter、敏感字段脱敏和测试
...
- Plus 模式不是固定一套步骤数量，前缀会随着 `PayPal / 无卡直绑 / GPC / 无需支付` 变化，尾链又会随着 `账号接入策略` 变化。
...
1. `PayPal`
   ...
   - 订阅回跳确认
2. `无卡直绑`
   - 创建 hosted Plus Checkout
   - 填写 PayPal 资料并完成 review
3. `GPC`
...
- Plus Checkout、PayPal、无卡直绑与 GPC 页面流程等待，都使用“可停止的后台等待”模型；等待过程中仍保持 Stop 可打断。
...
#### 7.1.2 DuckDuckGo 生成方式

Duck 邮箱生成只保留 DDG Token 直连链路：

1. 后台优先读取持久配置 `duckDdgToken`，直接调用 `https://quack.duckduckgo.com/api/email/addresses` 生成 `@duck.com` 地址。
2. 当 `duckDdgToken` 为空时，后台会打开 DuckDuckGo Email Protection 页面，通过 `chrome.webRequest.onBeforeSendHeaders` 监听 `quack.duckduckgo.com/api/email/addresses` 请求头，并触发一次 `FETCH_DUCK_EMAIL` 让页面产生真实 DDG API 请求；捕获到 `Authorization: Bearer ...` 后会自动回填并保存 `duckDdgToken`，再继续用 Token 直连生成邮箱。
3. 自动捕获 Token 需要用户已登录 DuckDuckGo 且已启用 Email Protection；未登录、未启用、监听能力缺失、请求失败或超时都会用中文错误停止当前邮箱生成，不会回退到页面生成。
4. DDG API 返回 429 或返回体中出现每日限额 / daily limit / quota 这类信息时，`background/duck-token-provider.js` 会抛出 `DDG_DAILY_LIMIT::` 标记错误；自动运行会把它视为整条流程硬停止，不进入当前轮重试，也不继续下一轮。
5. 这条链路只改变“生成 Duck 邮箱”的方式，不改变后续查信 provider；用户仍需保证 Duck 转发目标能被当前 `mailProvider` 收到验证码。
...
5. 执行 `runAutoSequenceFromStep`
   ...
   - 如果命中 DuckDuckGo 地址生成每日限制，则无论是否开启自动重试/跳过失败，都会直接停止整条自动运行，避免继续消耗 DDG 生成次数
...
8. 当前轮最终成功时，如果本轮是手机号注册且当前 flow 的全部 workflow node 都已经完成，后台会清空 `signupPhone* / accountIdentifier* / phoneNumber / currentPhoneActivation` 与 `email / registrationEmailState / step8VerificationTargetEmail` 等运行态，避免下一轮继续复用上一轮手机号或绑定邮箱
9. 如果配置了线程间隔，则挂计时计划；计时计划会带上当前 `autoRunSessionId`
10. 旧 timer / 旧 alarm / 旧恢复入口只有在 session 仍有效时才允许恢复执行；Stop 会立即使当前 session 失效，防止“停止后旧倒计时又把流程重新拉起”
11. 如果启用 IP 代理，后台会在每轮成功后按 `ipProxyPoolTargetCount` 检查是否需要切换出口；候选代理不足时只记录日志并跳过切换
12. 所有轮次结束后输出汇总，并清空当前 session 标识
...
3. [core/flow-kernel/step-registry.js](./core/flow-kernel/step-registry.js)
4. 如果步骤属于新 flow，还必须同步检查 [core/flow-kernel/flow-registry.js](./core/flow-kernel/flow-registry.js) 与 [core/flow-kernel/settings-schema.js](./core/flow-kernel/settings-schema.js)
4. 自动运行链路是否需要纳入
...
- 新增共享恢复层：`flows/openai/content/auth-page-recovery.js`。
