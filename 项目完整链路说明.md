...
- Plus capability 当前关闭，侧栏隐藏 Plus 开关、账号接入策略和支付配置；PayPal、无卡直绑与无需支付的底层步骤暂存，不会从用户界面或启动消息进入。
...
- Plus 支付方式 `plusPaymentMethod` 当前只保留 `paypal / paypal-hosted / none`，`plusModeEnabled` 在读取、导入、保存和启动时固定归一为 `false`。
...
Plus capability 当前关闭：`plusModeEnabled` 会被强制归一为 `false`，侧栏无入口，后台也不会启动 Plus。以下 PayPal、无卡直绑、无需支付与会话导入实现仅作为 dormant 底层代码暂存，供以后恢复时复用。

先说明边界：

- 暂存的 Plus 模式不是固定一套步骤数量，前缀会随着 `PayPal / 无卡直绑 / 无需支付` 变化，尾链又会随着 `账号接入策略` 变化。
...
- Plus Checkout、PayPal 与无卡直绑流程等待都使用“可停止的后台等待”模型；等待过程中仍保持 Stop 可打断。
